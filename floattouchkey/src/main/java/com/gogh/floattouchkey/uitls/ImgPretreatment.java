...
public class ImgPretreatment {
	...
	public static String doOcr(ImageView imageView, Bitmap bitmap){
		//将图片设置到mTess进行识别
		TessBaseAPI tessBaseAPI = TouchApplication.get().getTessAPI();
//		Bitmap orcBitmap = copy(doPretreatment(bitmap));
		Bitmap orcBitmap = copy(converyToGrayImg(bitmap));
		imageView.setImageBitmap(orcBitmap);
		tessBaseAPI.setImage(orcBitmap);
